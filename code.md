...
- 청산 우선순위: ①익절 → ②RSI 과매수 매도 → ③되돌림 익절 → ④트레일링 손절(연속 확인) → ⑤물타기.
- 되돌림 익절: 고점 수익률(peak_price 기준)이 `trailing_tp_arm_pct` 이상 갔다가 현재 수익률이
  `trailing_tp_floor_pct` 이하로 내려오면 매도 — 단 수익 구간(0% 초과)일 때만. 기본 꺼짐(3%/2%).
