...
기본 14)가 "과매수 기준"(기본 80) 이상이고 평단 대비 수익률이 **최소 수익률**(기본 +1%) 이상이면, 트레일링
손절 상태와 무관하게 **그 사이클에 바로 매도**한다(익절과 같은 우선순위 — 손절처럼 연속확인을 기다리지 않음).
수익률이 모자라면 RSI가 높아도 팔지 않고 아래 손절/익절 규칙을 그대로 따른다. 판단 우선순위는
①익절 → ②RSI 과매수 매도 → ③되돌림 익절 → ④트레일링 손절(연속확인) → ⑤물타기 순
(`app/core/trade_strategy.py`의 `evaluate_exits()` 참고).

최소 수익률은 2026-09-24에 추가됐다. 그 전에는 수익률 조건이 없어서 KRW-CVC가 45원에 사자마자 RSI 88.7로
44원(-3.7%)에 팔렸다 — RSI가 높다는 건 "많이 올랐다"는 뜻인데, 산 가격보다 낮으면 그 상승을 먹은 게
아니라 손절만 하나 더 생긴다. 0으로 두면 본전 이상에서만 판다.

회복형 분할 물타기 모드가 켜져 있으면 RSI 과매수 매도는 **수익 구간에서만** 동작한다(최소 수익률과 0% 중
큰 쪽, 아래 참고).
