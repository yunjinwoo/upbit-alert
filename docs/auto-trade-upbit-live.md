...
최고값이라 사이클 사이에 잠깐 튄 고가는 반영되지 않고, 되돌아왔는지 비교하는 현재 수익률도 그
사이클에 새로 조회한 시세 기준이라 사이클 사이에 급락하면 매도 기준보다 낮은 가격에 체결될 수 있다.
물타기가 실행되면 평단과 함께 `peak_price`도 리셋되므로 고점 수익률도 새 평단 기준으로 다시 쌓인다.
