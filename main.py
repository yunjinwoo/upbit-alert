...
if __name__ == "__main__":
    ...
    parser.add_argument("mode", nargs="?",
                        ...
                        help="Mode to run: all (default), api, upbit, stock, coin_analysis, trade, condition_check, "
                             "toss_analysis, toss_trade, toss_condition_check, live_balance, or live_trade")
    ...
    # live_trade(업비트 실거래 자동매매, 실주문 발생 가능)는 절대 all/start_all()에 포함하지 않고,
    # python main.py trade(모의)와도 완전히 별도 프로세스로만 실행 — 켜져 있어도 대시보드의
    # "실거래 실행" 스위치(기본 꺼짐)와 실거래 승인 체크박스가 둘 다 있어야 실제 주문이 나간다.
